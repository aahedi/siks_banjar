...
function loadLocal() {
    setTimeout(function() {
        ...
        if (sc === undefined || sc === null || sc.length === 0) {
			if(navigator.onLine){
            loadServer();
			}else{alert('Periksa koneksi internet Anda!');setTimeout(function(){location.reload()}, 3000);} 
		}else {
...
function loadServer() {
    ...
    xmlhttp.open("GET", "https://siks-banjar.cloudserver.id/android/js.php?" + Math.random(), true);
    xmlhttp.send();
}
//
